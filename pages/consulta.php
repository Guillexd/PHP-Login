...
?>
  <header>
      ...
      <div class="container">
          <a class="navbar-brand fw-bold py-1 px-0" href="./principal.php">Consulta total</a>
          ...
          <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav d-flex w-100 d-flex justify-content-between">
                <li class="nav-item me-5 p-2">
                  <a class="nav-link fw-bold py-1 px-0" aria-current="page" href="">Consulta por nombre</a>
                </li>
                <li class="nav-item ms-5 border border-info border-3 rounded-5 p-2">
                  <a class="nav-link active fw-bold py-1 px-0" href="../index.php" rel="noopener noreferrer">Cerar Sesión</a>
                </li>
            </ul>
          </div>
      </div>
  ...
  <main class="container d-flex flex-column align-items-center mt-5 h-100">
      <div class="text-center border border-5 p-5 rounded-4">
          <h1><span class="badge bg-secondary">Consulta por nombre</span></h1>
          <form action="" method="post">
              <label>Elija el Equipo:</label>
                  <select class="form-select form-select-lg mb-3" name="user">
              ...
              <input id="botoncito" class="btn btn-secondary" type="submit">
          </form><br>
      </div>
  ...
  <?php
        if (isset($_POST["user"])){
          ...
          $peticion = mysqli_query($conexion, $query);
          while ($dato= mysqli_fetch_assoc($peticion)){?>
